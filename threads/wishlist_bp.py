...
@wishlist_bp.route("/remove/<string:thread_id>", methods=['GET'])
@login_required
def remove(thread_id):
    ...
    if thread:
        if not repo_instance.is_in_favorite(username, thread):
            flash(f'{thread.thread_title} is not in your favorites!', 'info')
        else:
            repo_instance.remove_from_favorite(username, thread)
            flash(f'{thread.thread_title} has been removed from your favorites!', 'success')
    else:
        flash(f'{thread_id} does not exist!', 'danger')
    return redirect(request.referrer)
